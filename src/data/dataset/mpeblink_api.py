...
class MPEblink:
    ...
    def loadRes(self, resFile):
        ...
        res.dataset['videos'] = [img for img in self.dataset[
            'videos']]  # 'videos'中的信息如路径等是从train.json中拿过来的，因为先读了train.json，存在了self.dataset中了，这个直接拿过来和result.json没有任何冲突
        ...
        res.dataset[
            'annotations'] = anns  # 实际上就是把result.json的东西，整理的和train.json那样的格式，现在每个instance id代表的是一个query,也就是一个video有topk个query,一个video就占了topk个id
        res.createIndex()
        return res